...
Real support requires all of: `synthetic: false`, explicit `WALMART_CURRENT_SUPPORTED` mapping origin, compiled binding, and a trusted local golden evidence report with matching source/version/mapping hashes, fixture hash, PASS and named human review. Configure that report using `goldenEvidencePath`. The report is a local operator trust boundary, not a public upload or a Walmart certification. Analysis persists eligibility and provenance; checkout rechecks evidence and hashes, so missing/revoked evidence disables new checkout. Unknown, legacy and synthetic records cannot qualify. Provider tests use explicit eligibility mocks only, which are not golden evidence. Browser support controls and supported-template SEO claims use these gates. Local mock payments remain development-only.
