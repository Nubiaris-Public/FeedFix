...
## Deterministic pre-upload Error Decoder
The [Decoder catalog](error-decoder.md#initial-catalog-and-evidence) uses reviewed official Walmart guidance and separates DOCUMENTED, LIKELY_MATCH and UNKNOWN. It does not infer a spreadsheet mapping from a message, invoke Walmart APIs, modify schemas or promote compatibility. A fixed slug identifies each family; pasted values never become sources or SEO routes. Workbook/report correlation requires the existing supported mapping; metadata messages never identify a damaged cell without inspection. Its optional workbook CTA reuses NEW_WALMART_TEMPLATE and all existing upload outcomes. The independently consented error-message queue does not extend workbook study consent.
