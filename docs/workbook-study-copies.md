...
## Error-message study consent is separate
The pre-upload Decoder does not create a workbook study copy. Unknown error text is processed transiently unless its separate reviewed-text opt-in is accepted. That queue has no workbook, callback address, request metadata or contribution receipt and expires within 7 days; it cannot authorize longer workbook retention. See [error study privacy and operator requirements](error-decoder.md#privacy-and-retention). Browser replay/GA tags are disabled to protect the input surface. Existing workbook and notification TTLs remain unchanged.

Error-message sharing now displays the final text to be stored, resets consent after every edit and rejects any server submission that still needs redaction. This independent boundary does not alter workbook study receipts or retention.
